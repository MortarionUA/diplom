...
public class OrdersDataProvider extends FilterablePageableDataProvider<Order, Object> {
	...
	private LocalDate filterDate = LocalDate.now().minusDays(100);
	...
	protected Page<Order> fetchFromBackEnd(Query<Order, Object> query, Pageable pageable) {
		return orderService.findAnyMatchingAfterCreateDate(getOptionalFilter(), getOptionalFilterDate(), pageable);
	}
	...
	public void setIncludePast(boolean includePast) {
		if (includePast) {
			...
			filterDate = LocalDate.now().minusDays(100);
		}
	}

	@Override
	protected int sizeInBackEnd(Query<Order, Object> query) {
		return (int) orderService.countAnyMatchingAfterCreateDate(getOptionalFilter(), getOptionalFilterDate());
	}

	@Override
	protected List<QuerySortOrder> getDefaultSortOrders() {
		List<QuerySortOrder> sortOrders = new ArrayList<>();
		sortOrders.add(new QuerySortOrder("createDate", SortDirection.ASCENDING));
		sortOrders.add(new QuerySortOrder("createTime", SortDirection.ASCENDING));
