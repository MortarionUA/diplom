...
import com.gmail.dmytro.ui.components.ConfirmationDialogDesignElement;
import com.gmail.dmytro.ui.util.DollarPriceConverter;
import com.gmail.dmytro.ui.view.MenuElement;
import com.gmail.dmytro.ui.view.storefront.StorefrontViewElement;
...
public class AddOrderIT extends AbstractIT {
	...
	public static class TestOrder extends OrderEditViewElement.OrderInfo {
		public TestOrder() {
			...
			ProductInfoElement.ProductOrderData productOrderData = new ProductInfoElement.ProductOrderData("Strawberry Cheese Cake", 2, "Lactose free");
			// Price used only to verify that the UI is updated correctly
			productOrderData.setPrice(78.16);
			products.add(productOrderData);
			productOrderData = new ProductInfoElement.ProductOrderData("Vanilla Cracker", 1, "");
			// Price used only to verify that the UI is updated correctly
	...
	public void addOrder() {
		...
		OrderEditViewElement.OrderInfo testOrder = new TestOrder();
		...
		for (int i = 0; i < testOrder.products.size(); i++) {
			...
			ProductInfoElement.ProductOrderData product = testOrder.products.get(i);
			...
			Result<Double> itemPrice = new DollarPriceConverter().convertToModel(itemPriceText,
		...
		// Add + delete row
		orderEditView.getAddItems().click();
		ProductInfoElement.ProductOrderData product = testOrder.products.get(0);
	...
	public void changeStateForNewOrder() {
		...
		orderEditView
				.setProducts(Collections.singletonList(new ProductInfoElement.ProductOrderData("Blueberry Cheese Cake", 12, "A comment")));
		...
		orderEditView.getState().selectByText(OrderState.READY_TO_BOOK.getDisplayName());
		...
		orderEditView.getOk().click();
		Assert.assertEquals(OrderState.READY_TO_BOOK, orderEditView.getCurrentState());
	}
